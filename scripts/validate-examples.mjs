...
import { validateCustomRules, countEntitiesAndFields, formatValidationErrors } from './utils/validation-helpers.mjs';
...
const validateAllExamples = async () => {
  try {
    ...
    if (files.length === 0) {
      console.log('ℹ️  No .bprint files found in examples directory or subdirectories');
      return;
    }
    ...
    for (const file of files) {
      ...
      if (result.valid) {
        console.log(`✅ ${result.file} is valid (${result.entityCount} entities, ${result.fieldCount} fields)`);
